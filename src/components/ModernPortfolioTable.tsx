...
export const ModernPortfolioTable = ({
  securities,
  balances,
}: ModernPortfolioTableProps) => {
    ...
    result.sort((a, b) => {
      ...
      if (sortField === 'maturityDate') {
        aValue = a.maturityDate
          ? new Date(a.maturityDate.split('.').reverse().join('-')).getTime()
          : 0;
        bValue = b.maturityDate
          ? new Date(b.maturityDate.split('.').reverse().join('-')).getTime()
          : 0;
      }
