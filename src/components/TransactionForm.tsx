...
import { X, DollarSign, TrendingUp, TrendingDown, Plus, Minus, ChevronDown, Info } from 'lucide-react'
import DatePicker from './DatePicker'

interface TransactionFormProps {
  transaction?: Transaction | null
  onSubmit: (transaction: Omit<Transaction, 'id'>, continueAdding?: boolean) => void
  onCancel: () => void
  ...
  return (
    <div>
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        {isOpen && (
          <div className={`absolute z-50 w-full ${openUpward ? 'bottom-full mb-1' : 'top-full mt-1'} bg-white/95 dark:bg-gray-700/95 backdrop-blur-xl border border-gray-200/50 dark:border-gray-600/50 rounded-xl shadow-2xl max-h-60 overflow-auto`}>
            {options.map((option) => (
              <button
                key={option.value}
                type="button"
...
export const TransactionForm = ({ transaction, onSubmit, onCancel, transactions = [] }: TransactionFormProps) => {
  // Определяем начальный тип формы на основе транзакции
  const getInitialFormType = (): 'cash' | 'security' => {
    if (transaction) {
      const isCashOperation = ['deposit', 'debit', 'credit'].includes(transaction.type)
      return isCashOperation ? 'cash' : 'security'
    }
    return 'cash'
  }
  
  const [formType, setFormType] = useState<'cash' | 'security'>(getInitialFormType())
  ...
  const validateNumber = (value: string, fieldName: string, min = 0): string => {
    if (!value.trim()) {
    ...
    // Максимальное значение - 999,999,999,999,999,999
    const maxValue = 999999999999999999
    if (num > maxValue) {
      return 'Слишком большое число'
    }

    // Проверяем количество знаков после запятой (максимум 8 для криптовалют)
  ...
  const validateQuantity = (value: string): string => {
    ...
    // Проверка на целое число для облигаций
  ...
  const validateCashAmount = (value: string): string => {
    ...
    // Проверяем количество знаков после запятой (максимум 2 для денег)
    const decimalPlaces = (value.split('.')[1] || '').length
    if (decimalPlaces > 2) {
  ...
  const parseFormattedNumber = (value: string): string => {
    // Убираем разделители тысяч для валидации и отправки
  ...
  useEffect(() => {
    if (transaction) {
      ...
      // Используем price как номинальную стоимость, если nominalValue отсутствует
      const nominalValue = (transaction.security?.nominalValue || transaction.security?.price)?.toString() || ''
      const cashAmount = transaction.cash?.amount.toString() || ''
      
      // Если есть companyName, используем его, иначе пытаемся извлечь из name
      let companyName = transaction.security?.companyName || ''
      let bondName = transaction.security?.name || ''
      
      // Если companyName пустое, но name содержит название компании (например "ООО "ЗУБР АВТОГРУПП" 10.00%")
      if (!companyName && bondName) {
        // Пытаемся извлечь название компании из полного названия облигации
        const match = bondName.match(/^(.+?)\s+\d+\.\d+%$/)
        if (match) {
          companyName = match[1].replace(/^["']|["']$/g, '') // Убираем кавычки если есть
          bondName = '' // Очищаем название облигации, так как вся информация была в одном поле
        }
      }
      ...
      setFormattedValues({
        ...
        couponRate: transaction.security?.couponRate ? formatNumberInput(transaction.security.couponRate.toString()) : ''
      })
      
      // Тип формы уже определен в getInitialFormType()
    } else {
      // Сброс формы при создании новой транзакции
      setFormData({
        broker: 'finstore',
        type: '',
        date: new Date().toLocaleDateString('en-CA'), // YYYY-MM-DD format in local time
      ...
      setFormattedValues({ quantity: '', nominalValue: '', cashAmount: '', couponRate: '' })
      setFormType('cash')
    }
  }, [transaction])
  ...
  const handleSubmit = (e: React.FormEvent, continueAdding = false) => {
    ...
    if (formData.type === 'coupon') {
      // Валидация для купонного платежа
      ...
      const nominalValueError = validateNominalValue(formattedValues.nominalValue)
      ...
      if (companyNameError || bondNameError || quantityError || nominalValueError || couponRateError || maturityDateError) {
        hasErrors = true
      }
    } else {
      const cashAmountError = validateCashAmount(formattedValues.cashAmount)
    ...
    const baseTransaction = {
      ...
      type: formData.type as Transaction['type'], // Приводим к правильному типу после валидации
      date: formData.date.includes('T') ? formData.date : `${formData.date}T12:00:00`, // Добавляем время по умолчанию в местном времени
      note: formData.note.trim() || undefined
    }

    if (formData.type === 'coupon') {
      // Сохранение купонного платежа
      onSubmit({
        ...baseTransaction,
        security: {
      ...
      onSubmit({
        ...
        security: {
          ...
          price: formData.nominalValue ? parseFloat(formData.nominalValue) : 100, // Используем номинал как цену или 100 по умолчанию
          currency: formData.securityCurrency,
    ...
    // Reset form only if continuing to add
    if (continueAdding) {
      setFormData({
        broker: 'finstore',
        type: '',
        ...
        note: ''
            ...
            setErrors({ type: '', companyName: '', name: '', quantity: '', nominalValue: '', couponRate: '', maturityDate: '', cashAmount: '' })
      setFormattedValues({ 
        quantity: '', 
        cashAmount: '', 
        nominalValue: '', 
  ...
  const getOperationIcon = (type: Transaction['type']) => {
    switch (type) {
      case 'buy': return <Plus className="w-4 h-4" />
      case 'sell': return <Minus className="w-4 h-4" />
  ...
  // Получаем список активов в портфеле для купонного платежа
    ...
    transactions.forEach(t => {
      if (t.type === 'buy' && t.security) {
        ...
        if (!securitiesMap.has(symbol)) {
          securitiesMap.set(symbol, { symbol, name, companyName, currency, quantity: 0 })
        }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-start justify-center pt-8 p-4 z-50">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-gradient-to-tr from-emerald-400/20 to-blue-600/20 rounded-full blur-3xl animate-pulse" style={{animationDelay: '1s'}}></div>
      </div>
      ...
      <div className="relative bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 dark:border-gray-700/50 w-full max-w-md max-h-[calc(100vh-4rem)] overflow-y-auto scrollbar-hide">
        <div className="px-6 pt-6 pb-6">
          ...
          <form onSubmit={(e) => handleSubmit(e, false)} className="space-y-4 transition-all duration-200 ease-in-out">
            {/* Переключатель типа операции */}
            <div className="mb-6">
              <div className={`flex bg-gray-100/80 dark:bg-gray-700/50 p-1 rounded-xl backdrop-blur-sm ${transaction ? 'opacity-50 cursor-not-allowed' : ''}`}>
                <button
                  type="button"
                ...
                <button
                  ...
                  onClick={() => {
                    if (!transaction) {
                      ...
                      setFormData({ ...formData, type: '' as Transaction['type'] | '' })
                      setErrors({ type: '', companyName: '', name: '', quantity: '', nominalValue: '', couponRate: '', maturityDate: '', cashAmount: '' })
                      setFormattedValues({ quantity: '', nominalValue: '', cashAmount: '', couponRate: '' })
                    }
                  }}
            ...
            <div>
              <CustomDropdown
                label="Тип операции"
                value={formData.type}
                options={formType === 'security' ? securityOperations : cashOperations}
                onChange={(value) => {
            ...
            <div>
              <div className="flex items-center space-x-2 mb-2">
                ...
                <Tooltip text="Дата совершения операции">
                  <Info size={12} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" />
                </Tooltip>
              </div>
              <DatePicker
                value={formData.date}
              ...
              <div>
                ...
                <CustomDropdown
                  ...
                  onChange={(value) => {
                    ...
                    // Найдем выбранную бумагу и заполним данные
                    const selectedSec = getPortfolioSecurities().find(sec => sec.value === value)
                    if (selectedSec) {
                      // Найдем полную информацию о бумаге из транзакций
                      const secTransaction = transactions.find(t => 
                      ...
                      if (secTransaction?.security) {
                        setFormData(prev => ({
                          ...
                          companyName: secTransaction.security!.companyName || '',
                ...
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setFormattedValues({ ...formattedValues, cashAmount: formatted })
                      setFormData({ ...formData, cashAmount: parsed })
                      updateError('cashAmount', validateCashAmount(formatted))
                    }}
                  ...
                  />
                  {errors.cashAmount && (
                    <p className="mt-1 text-xs text-red-500">{errors.cashAmount}</p>
                  )}
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  ...
                  <CustomDropdown
                    ...
                    options={currencies}
                    onChange={(value) => setFormData({ ...formData, cashCurrency: value as 'BYN' | 'USD' | 'EUR' | 'RUB' })}
                    placeholder="USD"
                    openUpward={true}
                  />
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    updateError('companyName', validateCompanyName(e.target.value))
                  }}
                  className={`w-full px-3 py-2.5 bg-white/80 dark:bg-gray-700/80 backdrop-blur-sm border text-gray-900 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:border-transparent h-10 shadow-sm hover:shadow-md transition-all duration-200 ${
                    errors.companyName 
                      ? 'border-red-500/50 focus:ring-red-500/50 shadow-red-500/20' 
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({ ...formData, name: newName, symbol: newName.replace(/\s+/g, '_').toUpperCase() })
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const formatted = formatNumberInput(e.target.value)
                      const parsed = parseFormattedNumber(formatted)
                      setFormattedValues({ ...formattedValues, quantity: formatted })
                ...
                <div>
                  ...
                  <CustomDropdown
                    ...
                    options={currencies}
                    onChange={(value) => setFormData({ ...formData, securityCurrency: value as 'BYN' | 'USD' | 'EUR' | 'RUB' })}
                    placeholder="USD"
                    openUpward={true}
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const formatted = formatNumberInput(e.target.value)
                      setFormattedValues({ ...formattedValues, couponRate: formatted })
                      setFormData({ ...formData, couponRate: formatted })
                      updateError('couponRate', validateCouponRate(formatted))
                    }}
                    className="w-full px-3 py-2.5 bg-white/80 dark:bg-gray-700/80 backdrop-blur-sm border border-gray-200/50 dark:border-gray-600/50 text-gray-900 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent h-10 shadow-sm hover:shadow-md hover:border-gray-300/70 dark:hover:border-gray-500/70 transition-all duration-200"
                    placeholder="7.70"
                  />
                  {errors.couponRate && (
                    <p className="mt-1 text-xs text-red-500">{errors.couponRate}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    const formatted = formatNumberInput(e.target.value)
                    const parsed = parseFormattedNumber(formatted)
                    setFormattedValues({ ...formattedValues, nominalValue: formatted })
                    setFormData({ ...formData, nominalValue: formatted })
                    updateError('nominalValue', validateNominalValue(formatted))
                  }}
                    className="w-full px-3 py-2.5 bg-white/80 dark:bg-gray-700/80 backdrop-blur-sm border border-gray-200/50 dark:border-gray-600/50 text-gray-900 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent h-10 shadow-sm hover:shadow-md hover:border-gray-300/70 dark:hover:border-gray-500/70 transition-all duration-200"
                    placeholder="300"
                  />
                  {errors.nominalValue && (
                    <p className="mt-1 text-xs text-red-500">{errors.nominalValue}</p>
                  )}
                </div>
              </div>
