...
export const PortfolioChart = ({ securities }: PortfolioChartProps) => {
  const issuerMap = new Map<string, { name: string; value: number; currency: string }>()
  ...
  const getTotalSummary = () => {
    ...
    const parts = []
    if (currencyTotals.BYN > 0) parts.push(formatCurrency(currencyTotals.BYN, 'BYN'))
    if (currencyTotals.USD > 0) parts.push(formatCurrency(currencyTotals.USD, 'USD'))
    if (currencyTotals.EUR > 0) parts.push(formatCurrency(currencyTotals.EUR, 'EUR'))
    if (currencyTotals.RUB > 0) parts.push(formatCurrency(currencyTotals.RUB, 'RUB'))
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Распределение портфеля</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <PieChart>
                ...
                <Tooltip 
                  formatter={(value: number, name: string) => {
                    ...
                    return [formatCurrency(value, currency as 'BYN' | 'USD' | 'EUR' | 'RUB'), 'Сумма']
                  }}
        ...
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-gray-900 dark:text-white">Эмитенты</h4>
          {chartData.map((entry, index) => (
            <div key={entry.name} className="flex items-center justify-between p-2 rounded-lg bg-gray-50 dark:bg-gray-700">
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  {formatCurrency(entry.value, entry.currency as 'BYN' | 'USD' | 'EUR' | 'RUB')}
                </div>
              </div>
            </div>
          ))}
