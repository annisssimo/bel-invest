...
export const TransactionHistory = ({
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter((transaction) => {
        const companyName =
          transaction.security?.companyName?.toLowerCase() || '';
    ...
    if (filters.currencies.length > 0) {
      result = result.filter((transaction) => {
        const currency =
          transaction.security?.currency || transaction.cash?.currency;
    ...
    if (filters.dateTo) {
      result = result.filter(
        (transaction) => transaction.date <= filters.dateTo + 'T23:59:59'
      );
    }
    ...
    if (filters.amountFrom || filters.amountTo) {
      result = result.filter((transaction) => {
        ...
        if (filters.amountFrom && amount < parseFloat(filters.amountFrom))
          return false;
        if (filters.amountTo && amount > parseFloat(filters.amountTo))
          return false;
  ...
  const handleAmountFromChange = (value: string) => {
    ...
    setFilters((prev) => ({
      ...prev,
      amountFrom: parseFormattedNumber(formatted),
    }));
  };
  ...
  const handleAmountToChange = (value: string) => {
    ...
    setFilters((prev) => ({
      ...prev,
      amountTo: parseFormattedNumber(formatted),
    }));
  };
  ...
  if (transactions.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500 dark:text-gray-400">
        ...
        <p className="text-sm">
          Добавьте первую транзакцию для формирования портфеля
        </p>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(
                  ['buy', 'deposit', 'debit', 'coupon'] as Transaction['type'][]
                ).map((type) => (
                  <button
                    key={type}
              ...
              <div className="flex items-start space-x-2">
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-1.5">
                    <span
                      className={`px-2 py-0.5 text-xs font-medium border rounded-full ${getTypeColor(
                        transaction.type
                      )}`}
                    >
                        ...
                        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2 space-y-1.5">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-medium text-sm text-gray-900 dark:text-white">
                              {transaction.security.quantity} шт.
                            </span>
                          </div>
                  ...
                  {transaction.cash && (
                    <div className="mb-1.5">
                      <div className="text-base font-semibold text-gray-900 dark:text-white">
                        {['deposit', 'credit', 'coupon', 'dividend'].includes(
                          transaction.type
                        )
                          ? '+'
                          : '-'}
